...
import com.android.messaging.mmslib.util.MmsSmsUtils;
...
public class PduPersister {
    ...
    // We always use -1 as default/invalid sub id
    private static final Object DEFAULT_SUB_ID = -1;
    ...
    public static PduPersister getPduPersister(final Context context) {
        ...
        if (LOCAL_LOGV) {
            Log.v(TAG, "PduPersister getPduPersister");
        }
    ...
    public Uri persistPart(final PduPart part, final long msgId,
        ...
        if (LOCAL_LOGV) {
            Log.v(TAG, "PduPersister.persistPart part: " + uri + " contentType: " + contentType);
        }
            ...
            if (ContentType.TEXT_PLAIN.equals(contentType) && data != null) {
                ...
                if (encodedDataString != null && encodedDataString.startsWith(BEGIN_VCARD)) {
                    ...
                    if (Log.isLoggable(TAG, Log.DEBUG)) {
                        Log.d(TAG, "PduPersister.persistPart part: " + uri + " contentType: " +
    ...
    public Uri persist(final GenericPdu pdu, final Uri uri, final int subId,
        ...
        if ((msgType == PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND)
            ...
            if (!recipients.isEmpty()) {
                ...
                Log.w(TAG, "PduPersister.persist No recipients; persisting PDU to thread: "
                        + threadId);
            }
            values.put(Mms.THREAD_ID, threadId);
        }
        ...
        if (pdu instanceof MultimediaMessagePdu) {
            ...
            if (body != null) {
                final int partsNum = body.getPartsNum();
                if (LOCAL_LOGV) {
                    Log.v(TAG, "PduPersister.persist partsNum: " + partsNum);
                }
                if (partsNum > 2) {
                ...
                for (int i = 0; i < partsNum; i++) {
                    ...
                    if (LOCAL_LOGV) {
                        Log.v(TAG, "PduPersister.persist part: " + i + " contentType: " +
                                contentType);
                    }
        ...
        if (version >= Build.VERSION_CODES.LOLLIPOP_MR1) {
            ...
            if(DEFAULT_SUB_ID != subId){
                Log.e(TAG, "Expected " + DEFAULT_SUB_ID + " but got " + subId);
            }
        }
